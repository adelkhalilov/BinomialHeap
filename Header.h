...
class binomial_heap
{
	...
	public:
		...
		tree_node* tree_merge(tree_node* new_tree) //сливаем два дереве одинаковой степени в одно
		{
			...
			{
				throw logic_error("Trees have different degrees!");
			}
...
public:
	...
	void change(std::shared_ptr<Pointer> pointer, Key new_value)
	{
		if (!pointer->is_mark())
		{
			throw std::runtime_error("Vetrex is already deleted!");
		}
	...
	void erase(std::shared_ptr<Pointer> pointer)
	{
		if (!pointer->is_mark())
		{
			throw runtime_error("Vetrex is already deleted!");
		}
...
private:
	...
	void cut_out_root(tree_node* prev_root, tree_node* current_root)
	{
		...
		{
			throw runtime_error("Roots are not subsequent!");
		}
